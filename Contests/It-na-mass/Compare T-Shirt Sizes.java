...
any small size (no matter how many letters X) is smaller than the medium size and any large size; any large size (regardless of the number of letters X) is larger than the medium size and any small size; the more letters X before S, the smaller the size; the more letters X in front of L, the larger the size. For example:

XXXS < XS XXXL > XL XL > M XXL = XXL XXXXXS < M XL > XXXS

Input Format
The first line of the input contains a single integer t (1≤t≤10^4) — the number of test cases.Each test case consists of one line, in which a and b T-shirt sizes are written. The lengths of the strings corresponding to the T-shirt sizes do not exceed 50. It is guaranteed that all sizes are correct.
...
public class Main {
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            ...
            if (a.equals(b)) {
                System.out.println("=");
            } else if (lastA == 'M' && (lastB == 'S' || lastB == 'L')) {
                System.out.println(lastB == 'S' ? ">" : "<");
            } else if (lastB == 'M' && (lastA == 'S' || lastA == 'L')) {
                System.out.println(lastA == 'S' ? "<" : ">");
            } else if (lastA == 'S' && lastB == 'S') {
                // more X means smaller
                if (a.length() > b.length()) {
                    System.out.println("<");
                } else {
                    System.out.println(">");
                }
            } else if (lastA == 'L' && lastB == 'L') {
                // more X means larger
                if (a.length() > b.length()) {
                    System.out.println(">");
                } else {
